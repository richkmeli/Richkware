...
void Blowfish::SetKey(const char *keyT, size_t byte_length) {
...

    {
        ...
        for (unsigned long i = 0; i < buffer_length; ++i) {
            ...
            converter.bit_8.byte0 = keyT[(i * 4) % byte_length];
            converter.bit_8.byte1 = keyT[(i * 4 + 1) % byte_length];
            converter.bit_8.byte2 = keyT[(i * 4 + 2) % byte_length];
            converter.bit_8.byte3 = keyT[(i * 4 + 3) % byte_length];
...
std::string Blowfish::Encrypt(const std::string &src) const {
    ...
    std::string dstS(dst.begin(), dst.end());
    return dstS;
}
